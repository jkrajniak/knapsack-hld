...
def main() -> int:
    ...
    print(
        f"  {'corr':23s} {'f':4s} |  n  help hurt neut | mean_delta  min_delta  max_delta | po_fill_range"
    )
    for k in sorted(buckets):
        ...
        print(
            f"  {k[0]:23s} {k[1]:.2f} | {len(rs):3d}  {nh:4d} {nu:4d} {nn:4d} | "
    ...
    for ft in [60, 70, 80, 85, 90, 92, 95, 97, 99, 100]:
        ...
        print(
            f"  {ft:4d} | {len(skip):5d} {len(run):5d} | {len(sh):3d} {mh:7.2f}  "
    ...
    for r in sorted(rows, key=lambda x: -x["delta"])[:25]:
        if r["hurt"]:
            print(
                f"  {r['corr']:23s} {r['f']:.2f} {r['m']:3d} {r['bs']:3d} | "
